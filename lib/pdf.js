...
const VENUE_LABELS = {
  telephone: 'Telephone', email: 'Email', zoom: 'Zoom', whatsapp: 'WhatsApp',
  inPerson: 'In Person', letter: 'Letter', other: 'Other'
};

const SAMPLE_LABELS = {
  video: 'sample teaching video',
  lessonPlan: 'lesson plan',
  observation: 'live class observed'
};
...
function _interviewHasData(iv) {
  return Boolean(iv && (iv.interviewerName || iv.interviewerTitle || iv.date || iv.time ||
...
function _totalInterviewMinutes(t) {
  return _usedInterviews(t).reduce((sum, iv) => {
...
function _interviewVenueStr(iv) {
  ...
  return VENUE_LABELS[iv.venue] || '';
}

function _teachingSampleStr(t) {
  if (t.teachingSampleProvided === 'no') return 'No';
  if (t.teachingSampleProvided !== 'yes') return '';
  const provided = (t.teachingSampleTypes || []).map(type => SAMPLE_LABELS[type] || type);
  const needsDuration = (t.teachingSampleTypes || []).some(x => x === 'video' || x === 'observation');
  const duration = needsDuration && t.teachingSampleMinutes ? ` (${t.teachingSampleMinutes} minutes)` : '';
  return `Yes - ${provided.join(', ')}${duration}`;
}

function _minutesStr(minutes) {
  const value = String(minutes ?? '').trim();
  if (!value) return '';
  return Number.isFinite(Number(value)) ? `${value} minutes` : value;
}
...
export class PDFGenerator {
  ...
  drawParagraph(label, text) {
    const value = this._sanitize(text);
    if (!value) return;
    this.addSpace(4);
    this.addPageIfNeeded(this.fonts.body * 3);
    this.currentPage.drawText(this._sanitize(label) + ':', {
    ...
    this.drawWrappedText(value, this.margins.left + 15, this.contentWidth - 20);
  }
  ...
  drawSummaryTable(teachers) {
    const headers = ['Teacher Name', 'Email', 'First Interview', 'Interviews'];
    const rows = teachers.map(t => {
      const used = _usedInterviews(t);
      return [
        `${t.firstName} ${t.lastName}`,
        t.email,
        _firstInterviewDate(t),
        used.length ? `${used.length} / ${_totalInterviewMinutes(t)} min` : ''
      ];
    });
  ...
  async generate(report, onProgress = () => {}) {
    ...
    for (let i = 0; i < report.teachers.length; i++) {
      ...
      if (t.citizenshipCountry) this.drawField('Citizenship', t.citizenshipCountry);

      const interviews = _usedInterviews(t);
      if (interviews.length > 0) {
        this.addSpace(5);
        this.addPageIfNeeded(this.fonts.body * 1.6 + 5);
        this.currentPage.drawText('Interviews:', {
          x: this.margins.left + 5, y: this.currentY - this.fonts.body,
          size: this.fonts.body, font: this.fontBold,
          color: rgb(this.colors.primary.r, this.colors.primary.g, this.colors.primary.b)
        });
        ...
        for (const iv of interviews) {
          ...
          const who = iv.interviewerTitle ? `${iv.interviewerName} (${iv.interviewerTitle})` : iv.interviewerName;
          const parts = [when, _minutesStr(iv.minutes), _interviewVenueStr(iv), who ? `interviewed by ${who}` : '']
            .filter(Boolean);
          this.drawWrappedText(`- ${parts.join(', ')}`, this.margins.left + 15, this.contentWidth - 15);
        }
      }
      this.drawField('Total Interview Time', _minutesStr(_totalInterviewMinutes(t)));

      this.drawParagraph('How the teacher was recruited', t.recruitmentMethod);
      this.drawParagraph('How the candidate was evaluated', t.candidateAssessment);

      const sample = _teachingSampleStr(t);
      if (sample) this.drawField('Teaching sample provided', sample);
      if (t.teachingSampleProvided === 'no') {
        this.drawParagraph('How classroom ability was evaluated instead', t.teachingSampleAlternative);
      }
      ...
      for (let r = 0; r < (t.references || []).length; r++) {
        ...
        if (ref.interviewDate) {
          this.drawField('  Spoken to on', [ref.interviewDate, ref.interviewTime].filter(Boolean).join(' at '));
        }
        if (ref.interviewMinutes) this.drawField('  Length of reference check', _minutesStr(ref.interviewMinutes));
        ...
        this.drawParagraph('  What the reference said', ref.feedback);
      }
